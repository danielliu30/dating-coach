...
- Sessions are a short-lived access JWT (15m) plus a rotating refresh token stored hashed in PostgreSQL. Authenticating a request is pure signature checking — no database round trip — and ending a session means deleting its refresh tokens: `POST /api/v1/auth/refresh` rotates one into a new pair, and replaying a spent token drops every refresh token of that account.
- Account deletion is two-phase: `DELETE /api/v1/auth/me` stamps `users.deleted_at` and deletes the account's refresh tokens before it answers, so sign-in refuses the account like an unknown one and the session cannot be renewed past the few minutes its access token has left, then queues the row removal (cascading across every table) to the worker. A failed attempt waits 30s on `account.deletion.retry` before it is redelivered, and deletions that keep failing land on `account.deletion.dlq`. The worker logs that queue's depth every `DEAD_LETTER_ALERT_PERIOD`; alert on a non-zero depth, because those accounts can no longer sign in but still hold rows.
