...
- Account deletion is split: `DELETE /api/v1/account` writes the account to a Redis denylist (TTL `JWT_TTL`) before responding and queues the cascading row delete on `ACCOUNT_DELETION_QUEUE`. The auth middleware checks the denylist on every request, so outstanding tokens are rejected with 401 as soon as the call returns, whenever the worker gets to the rows. If queueing fails the denylist entry is removed again and the request errors, so the account can retry instead of being locked out of a deletion that was never scheduled. A failing delete waits 30s in `account.deletion.retry` between attempts (counted in a message header, not the broker's redelivered flag) and lands on `account.deletion.dead` via the dead-letter exchange after three handler failures; the worker logs that queue's depth every `DEAD_LETTER_ALERT_PERIOD` and **alerting on it is required** — a stuck delete is otherwise invisible once the denylist entry expires and the rows are still there.
