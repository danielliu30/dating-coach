...
- Account deletion goes through an outbox: `DELETE /api/v1/auth/me` stamps `users.deleted_at` and records the deletion in `account_deletions` in one statement, which is the only failure the caller is told about — once it commits the deletion is certain, because a relay in the worker queues every recorded deletion the request itself does not. Sign-in then refuses the account like an unknown one. Revoking the account's refresh tokens and publishing the job directly are optimisations the request does on a best-effort basis (they only log on failure): the revoke stops any further access token from being minted immediately instead of waiting for the worker, and the direct publish skips the relay's next pass. The worker deletes the row (cascading across every table, refresh tokens included) and clears the outbox entry; the access token the caller already holds dies within 15 minutes either way. A failed attempt waits 30s on `account.deletion.retry` before it is redelivered, and deletions that keep failing land on `account.deletion.dlq`. The worker logs that queue's depth every `DEAD_LETTER_ALERT_PERIOD`; alert on a non-zero depth, because those accounts are marked deleted but still hold rows.
- A chat socket outlives the request that authenticated it, so it re-checks the account row every 15s (and on every client event), closes itself when the access token that opened it expires, and is dropped at once when a deletion is announced on Redis pub/sub — the announcement is only a latency optimisation over that durable re-check. A socket closed because the account is gone signs the app out; one closed on expiry does not, since the app renews its own access token.
